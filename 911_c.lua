RegisterNetEvent('el-911:client:createBlip')
AddEventHandler('el-911:client:createBlip', function(type, coords, name, message, id, source)
    plyData = ELCore.Functions.GetPlayerData()
...
RegisterNetEvent('el-911:client:reply')
AddEventHandler('el-911:client:reply', function(type, id, message,officer, source)
	if ((PlayerJob.name == "police" or PlayerJob.name == "ambulance") and PlayerJob.onduty) or (source == GetPlayerServerId(PlayerId())) then
...
RegisterNetEvent('el-911:client:justcalled')
AddEventHandler('el-911:client:justcalled', function()
	if ELCore.Functions.GetPlayerData().metadata["isdead"] or IsEntityPlayingAnim(PlayerPedId(), "cellphone@", "cellphone_call_listen_base", 3) or IsPedRagdoll(PlayerPedId()) then
		return
	end
...
RegisterCommand('lfmso', function()
	local nearbyVehicles = ELCore.Functions.GetVehicles()
